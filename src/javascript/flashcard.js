...
const askAI = document.getElementById("ask-ai-button");
...
askAI.addEventListener("click", generateAIPrompt);
...
function generateAIPrompt() {
    ...
    const number = parseInt(prompt("Enter the number of questions for the quiz:", 15));

    // Create the prompt template with the user's input
    const promptTemplate = `
    I would like to create a quiz based on the following topic or lesson content. Please generate a JSON object with the title of the quiz and at least ${number} question-answer pairs. The format should follow the example provided below, with each question derived from the information in the text. Here's the lesson content or topic description:

    ${content}

    Example format for the output JSON:
    ...
    Ensure the JSON is structured correctly, and questions are relevant to the content.`;
